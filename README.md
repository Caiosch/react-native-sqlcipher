...
Forked from [linchenhuicn/react-native-sqlcipher](https://github.com/linchenhuicn/react-native-sqlcipher) for updating, because it didn't receive any updates during the last two years and I needed an updated project.
...
Version 0.0.7 is forked from [andpor/react-native-sqlite-storage](https://github.com/andpor/react-native-sqlite-storage/) #6.0.1, Sqlcipher#4.5.0
...
```js
DB.openDatabase(
  {name: 'my.db', key: 'yourPassword', location: 'default'},
  successcb,
  errorcb,
);
```
...
```js
DB.openDatabase(
  {name: 'my.db', key: 'yourPassword', location: 'Library'},
  successcb,
  errorcb,
);
```
...
```js
DB.openDatabase(
  {name: 'testDB', key: 'yourPassword', createFromLocation: 1},
  okCallback,
  errorCallback,
);
```
...
dbSecond = DB.openDatabase(
  {name: 'second', key: 'yourPassword'},
  (db) => {
    dbMaster = DB.openDatabase(
      {name: 'master', key: 'yourPassword'},
      (db) => {
        dbMaster.attach(
          'second',
          'second',
...
```javascript
DB.copyDBFile(
  {name: 'test.db', key: 'yourPassword', createFromLocation: 1},
