...
var DB_STATE_INIT, DB_STATE_OPEN, READ_ONLY_REGEX, SQLiteFactory, SqlcipherPlugin, SQLitePluginTransaction, argsArray, dblocations, newSQLError, root, txLocks;
...
SqlcipherPlugin = function(openargs, openSuccess, openError) {
  var dbname;
  if (!(openargs && openargs["name"])) {
    throw newSQLError("Cannot create a SqlcipherPlugin db instance without a db name");
  }
...
SqlcipherPlugin.prototype.databaseFeatures = {
  isSQLitePluginDatabase: true
};

SqlcipherPlugin.prototype.openDBs = {};

SqlcipherPlugin.prototype.addTransaction = function(t) {
...
SqlcipherPlugin.prototype.transaction = function(fn, error, success) {
...
SqlcipherPlugin.prototype.readTransaction = function(fn, error, success) {
  if (!this.openDBs[this.dbname]) {
    error(newSQLError("database not open"));
    return;
  }
...
SqlcipherPlugin.prototype.startNextTransaction = function() {
  var self;
  self = this;
...
SqlcipherPlugin.prototype.abortAllPendingTransactions = function() {
...
SqlcipherPlugin.prototype.sqlBatch = function(sqlStatements, success, error) {
...
SqlcipherPlugin.prototype.open = function(success, error) {
...
SqlcipherPlugin.prototype.close = function(success, error) {
  if (this.dbname in this.openDBs) {
...
SqlcipherPlugin.prototype.attach = function(dbNameToAttach, dbAlias, success, error) {
  if (this.dbname in this.openDBs) {
    if (txLocks[this.dbname] && txLocks[this.dbname].inProgress) {
...
SqlcipherPlugin.prototype.detach = function(dbAlias, success, error) {
  if (this.dbname in this.openDBs) {
    if (txLocks[this.dbname] && txLocks[this.dbname].inProgress) {
      plugin.log("cannot attach: transaction is in progress");
      error(newSQLError("database cannot be attached while a transaction is in progress"));
...
SqlcipherPlugin.prototype.executeSql = function(statement, params, success, error) {
...
SQLiteFactory.prototype.openDatabase = argsArray(function(args) {
  ...
  return new SqlcipherPlugin(openargs, okcb, errorcb);
});
...
SQLiteFactory.prototype.deleteDatabase = function(first,success, error) {
  ...
  let mysuccess = function(r) {
    delete SqlcipherPlugin.prototype.openDBs[args.path];
    if (!!success) {
      return success(r);
    }
  };
...
plugin.sqlitePlugin = {
  ...
  SqlcipherPlugin : SqlcipherPlugin,
  log: plugin.log
};
